...
const MyResults = () => {
    ...
    useEffect(() => {
        const fetchResults = async () => {
            try {
                const url =
                    process.env.REACT_APP_BACKEND_URL + "/quizzes/taken";
                console.log(url);
    ...
    return (
        <>
            {Loading && <PageLoader />}
            {!Loading && (
                <div className="quiz-results-container bg-[#0d1b2a] rounded-xl ml-[5px] h-[88vh] w-[80vw] fixed overflow-scroll">
                            ...
                            >
                                <div className="card-content">
                                    ...
                                    <div className="card-text">
                                        Total Questions:{" "}
                                        {quizResult.numQuestions}
                                    </div>
                                    <div className="card-text score-text">
                                        Score: {quizResult.quizScore}/
                                        {quizResult.numQuestions}
                                    </div>
                                    <button
                                        className="stats-button"
                                        onClick={() =>
                                            (window.location.href = `/result/${quizResult.quiz_id}`)
                                        }
                                    >
                                        Stats &nbsp;
                                        <span>
                                            <iconify-icon icon="arcticons:spotistats"></iconify-icon>
                                        </span>
                                    </button>
                                </div>
                            </div>
