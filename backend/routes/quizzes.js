...
router.post(
  ...
  catchAsync(async (req, res, next) => {
    ...
    if (!title || !questions || questions.length === 0)
      return next(new AppError("Title and questions are required", 400));

    let existingQuiz = await Quiz.findOne({ quiz_id });
    if (existingQuiz) return next(new AppError("Quiz ID already exists", 400));
  ...
  catchAsync(async (req, res, next) => {
    ...
    for (const answer of answers) {
      ...
      if (!question)
        return next(
          new AppError(
            `Question with ID ${question_id} not found in the quiz`,
            404
          )
        );
...
router.get(
  ...
  catchAsync(async (req, res, next) => {
    ...
    if (!quizProgress) return next(new AppError("Progress Not Found", 404));
